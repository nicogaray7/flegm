...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonical = getCanonicalForLocale(locale as Locale, "top/month");
  return {
    ...
    alternates: { canonical, languages: getAlternateLanguages("top/month") },
    openGraph: {
...
export default async function TopMonthPage({ params }: Props) {
        ...
        {topVideos.length === 0 ? (
          <div className="rounded-2xl border-2 border-dashed border-purple-200 bg-purple-50/50 px-6 py-8 text-center">
            <p className="text-sm text-[var(--muted)]">
              {t.topMonth.empty} <Link href={`/${locale}/submit`} className="font-bold text-[var(--accent)] hover:underline">{t.topMonth.emptyLink}</Link>
            </p>
          </div>
        ) : (
